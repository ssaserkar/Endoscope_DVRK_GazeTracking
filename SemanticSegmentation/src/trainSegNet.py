...
parser.add_argument('--dataset', type=str, help='dataset title (options: cholec)')
...
parser.add_argument('--differential_lr', default=False, type=bool, help='use differential learning rate for '
...
def validate(val_loader, model, criterion, dice_loss, epoch, key, evaluator, losses, img_mean, img_std, logger, args):
    ...
    for i, (img, gt, label) in val_loop:
        # Process the network inputs and outputs
        img = utils.normalize(img, torch.Tensor(img_mean), torch.Tensor(img_std))
        oneHotGT = one_hot(label, 13).permute(0, 3, 1, 2) #len(key)
